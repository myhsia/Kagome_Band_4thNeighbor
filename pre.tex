...
\usepackage{graphicx, mathtools, tikz, menukeys, physics3,
...
\title[Final Project Presentation]{Band Structure of 2D Kagome Lattice}
\author[Mingyu Xia]{\texorpdfstring{Mingyu Xia (夏明宇) |
                    \textit{Department of Physics, Westlake University}}
                      {Mingyu Xia}}
\date{\texttt{2026-01-09 | E10-215@Yungu Campus}}
  ...
  \begin{block}{Coined by Kôdi Husimi}
    ...
    \begin{column}{.72\linewidth}
      ...
      \begin{itemize}
        ...
        \item Combined rotation-reflection ($C_{6\sigma}$) operations
      \end{itemize}
    \end{column}
    \hfill
...
\begin{frame}{Lattice Structure}
  \begin{block}{Three type of sites}
    \begin{itemize}
      ...
      \item Hops' magnitude between different types of sites is different
    \end{itemize}
  \end{block}
  \begin{figure}[htbp]
...
\begin{frame}{Dispersion Relation}
  ...
  \begin{exampleblock}{Eigen Equation}
    From $\det(\tilde{\mathcal H} - E\identity) = 0$, one
...
\begin{frame}{An Extra NNN or NNNN Hopping}
  ...
  be obtained from $\det(\tilde{\mathcal H} - E\identity) = 0$.}%
\end{frame}
